...
The app configuration specifies iOS version `1.0`, build `2`, and bundle ID `com.duoarcade.app`. A Release device archive and App Store distribution IPA for build 2 were generated successfully with Xcode 26.6 on September 16, 2026. Build 1 has a historical successful Apple upload record; build 2 upload and processing are not yet verified. Native touch, lifecycle, and cross-platform acceptance remain partially complete; compilation is not a claim that all games passed. `pnpm build:ios` exports JavaScript/assets; it does not produce a signed App Store archive. The current branch's catalog update is not a claim about the deployed web or iOS release.
